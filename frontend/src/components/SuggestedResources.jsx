...
const SuggestedResources = () => {
  ...
  const fetchReport = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:8000/assist/${userId}/suggestresources`);
      setReport(response.data.suggestResources);
      setIsOpen(true);
    } catch (error) {
      ...
      <div className="relative p-8 bg-gray-900/40 backdrop-blur-xl rounded-lg border border-white/10 shadow-2xl">
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-300 mb-6 leading-relaxed">
            Click the button below to get your Resource.
          </p>
          <button
            onClick={fetchReport}
            disabled={loading}
