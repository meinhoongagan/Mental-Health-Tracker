...
const CopingStrategies = () => {
  ...
  const fetchReport = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:8000/assist/${userId}/copingstrategies`);
      setReport(response.data.copingStrategies);
      setIsOpen(true);
    } catch (error) {
      ...
      <div className="relative p-8 bg-gray-900/40 backdrop-blur-xl rounded-lg border border-white/10 shadow-2xl">
        ...
        <div className="relative z-10">
          <h3 className="text-xl font-semibold mb-4 text-white/90 tracking-wide">
            Best Coping Strategies For You
          </h3>
          <p className="text-gray-300 mb-6 leading-relaxed">
            Click the button below to get your Coping Strategies.
          </p>
          <button
            onClick={fetchReport}
            disabled={loading}
